...
lista_divisores(X, Y, [Y|Resto]) :- Y > 1, Y2 is Y-1, lista_divisores(X, Y2, Resto), 0 is X mod Y.
lista_divisores(X, Y, R) :- Y > 1, Y2 is Y-1, lista_divisores(X, Y2, Resto), Z is X mod Y, Z \== 0.
